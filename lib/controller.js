...
class TorController extends EventEmitter {
  ...
  /**
   * Instruct Tor to load the configuration file from the given path
   * @param {string} configFilePath - Path to the config to read and send
   * @param {TorController~loadConfigCallback} callback
   */
  loadConfig(configFilePath, callback) {
    let configText = null;

    try {
      configText = readFileSync(configFilePath).toString();
    } catch (err) {
      return callback(err);
    }
